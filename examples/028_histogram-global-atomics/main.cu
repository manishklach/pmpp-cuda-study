...
std::vector<unsigned int> make_input(int n, unsigned int seed) {
  ...
  for (int i = 0; i < n; ++i) {
    // Most values land in a small hot set of bins so the global-atomic contention is visible.
...
__global__ void histogram_global_kernel(const unsigned int *input, unsigned int *bins, int n) {
  ...
  // Threads read the input linearly, but the bin update is an irregular write. When many
  // values map to the same bin, atomics serialize and become the dominant bottleneck.
...
pmpp::ValidationSummary run_check(const pmpp::CommonOptions &options) {
  ...
  const int threads = sanitize_block_size(options.block_size);
  ...
  summary.notes =
      "This direct baseline is simple and correct, but hot bins create heavy global-atomic "
      "contention.";
  return summary;
}
