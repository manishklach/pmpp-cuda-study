...
__global__ void k(const uchar3 *in, uchar3 *out, int w, int h) {
  int x = blockIdx.x * blockDim.x + threadIdx.x;
  int y = blockIdx.y * blockDim.y + threadIdx.y;
  if (x < w && y < h) {
    int idx = y * w + x;
    uchar3 p = in[idx];
    out[idx] = make_uchar3(255 - p.x, 255 - p.y, 255 - p.z);
  }
}
int main() {
  ...
  for (int i = 0; i < c; ++i) {
    ...
    uchar3 p = in[i];
    co[i] = make_uchar3(255 - p.x, 255 - p.y, 255 - p.z);
  }
